...
module.exports = {
  activate() {
    ...
    this.disposables = new CompositeDisposable(
      atom.commands.add("atom-workspace", {
        "open-external:toggle": () => this.toggleWatcher(),
      }),
      atom.commands.add("atom-text-editor:not([mini]), .image-view, .image-editor, .pdf-viewer", {
        "open-external:open": () => this.itemOpenExternal(),
        "open-external:show": () => this.itemShowInFolder(),
      }),
  ...
  toggleWatcher() {
    const value = !atom.config.get("open-external.flag");
    atom.config.set("open-external.flag", value);
    if (value) {
  ...
  registerHandler(handler) {
    if (!handler || typeof handler.priority !== "number") {
      throw new Error("Handler must have openExternal() or showInFolder() and priority");
    }
